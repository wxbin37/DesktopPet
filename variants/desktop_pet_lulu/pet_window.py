...
class PetWindow(QWidget):
    def __init__(self, keyboard_enabled=True):
        ...
        self.typing_click_frame = self.typing_scene_base.copy()
        if not self.typing_click_frame.isNull():
            mask_painter = QPainter(self.typing_click_frame)
            mask_painter.drawPixmap(0, 0, self.typing_rest_overlay)
            mask_painter.drawPixmap(0, 0, self.typing_tap_overlay)
            mask_painter.end()
    ...
    def _get_display_frame(self):
        ...
        if (
            ...
            return self.typing_click_frame.scaled(
                self.size(),
    ...
    def _draw_bongo_keyboard(self, painter):
        ...
        target = self._target_rect_for_source(self.typing_scene_base)
        keyboard = QRectF(target.x() + target.width() * 0.35, target.y() + target.height() * 0.755, target.width() * 0.60, target.height() * 0.20)
