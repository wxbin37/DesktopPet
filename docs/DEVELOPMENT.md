...
当前走路动作已经从原来的 6 帧 A/B 循环升级为 8 帧步态循环，并让窗口位移计时与 `FPS` 对齐。这样可以减少“岔开腿图片一直闪、身体却在滑”的感觉。

当前已做：

- `walking` 从 6 帧增加到 8 帧
- 用接触、回弹、抬脚、换脚的节奏组织帧
- 对源图加入轻微倾斜、压缩/伸展和上下起伏
- `WALK_STEP_INTERVAL = int(1000 / FPS)`，让走路位移和动画帧同步

受限于源角色表只有 `walk_a` 和 `walk_b` 两个走路姿势，这仍然不是逐关节动画。若想继续提升真实感，推荐方向：

- 从 2 个 walk 姿势扩展到 4 个或 6 个基础姿势
- 在角色源表中提供接触、下压、经过、抬脚等中间帧
- 给身体增加轻微上下起伏
- 手臂与腿做相反方向摆动
