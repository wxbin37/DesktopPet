...
当前走路动作已经从原来的 A/B 两张图循环，改为独立 4×2 walking sprite sheet。这样腿部动作来自真实分帧，而不是靠整张图缩放、拉伸或旋转。

当前已做：

- `art/walking_sprite_sheet_alpha.png` 存在时，优先用它生成 walking
- `walking` 为 8 帧，腿部真实交替
- walking 8 帧使用同一个缩放比例，避免每帧因外框不同而忽大忽小
- walking 以脚底为锚点对齐，减少上下乱跳
- 清理脚底残影、倒影，以及相邻 sprite 格子漏出的孤立碎片
- `WALK_STEP_INTERVAL = int(1000 / FPS)`，让走路位移和动画帧同步

如果想继续提升真实感，推荐方向：

- 重新绘制或生成更稳定的 `art/walking_sprite_sheet_alpha.png`
- 在 8 帧中明确提供接触、下压、经过、抬脚、换脚等中间帧
- 给身体增加轻微上下起伏
- 手臂与腿做相反方向摆动
- 转向时保持同一帧镜像，不重新生成另一套人物
- 保持每帧脚底位置稳定，避免人物像在“闪烁跳帧”

原则：代码可以控制播放节奏、位移、锚点和清理，但不能凭空把一张静态腿变成真实逐关节运动。要自然，素材本身必须有真实腿部变化。
